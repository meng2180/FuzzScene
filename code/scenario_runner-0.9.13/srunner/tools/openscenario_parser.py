...
class OpenScenarioParser(object):
    ...
    def get_weather_from_env_action(xml_tree, catalogs):
        ...
        if carla_weather.fog_distance >=20:
            # carla_weather.fog_density=25-0.125*(float(carla_weather.fog_distance))
            carla_weather.fog_density=(200-float(carla_weather.fog_distance))/9
        else:
            # carla_weather.fog_density=100-2*(float(carla_weather.fog_distance))
            carla_weather.fog_density=100-4*(float(carla_weather.fog_distance))
        print(carla_weather.fog_distance,carla_weather.fog_density)
        ...
        if precepitation.attrib.get('precipitationType') == "rain":
            precipitation = float(precepitation.attrib.get('intensity')) * 100
            if precipitation<=60:
                carla_weather.precipitation=precipitation*0.5
            else:
                # carla_weather.precipitation=precipitation*(4/3.0)-(100/3.0)
                carla_weather.precipitation=precipitation*(7/4.0)-75
            carla_weather.precipitation_deposits = carla_weather.precipitation  # if it rains, make the road wet
            print(carla_weather.precipitation_deposits)
            #TODO
